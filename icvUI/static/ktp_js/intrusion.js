...
$(function(){
    ...
    let get_history_url = '/get_single_intrusion_history?camera=' + camera;
...
function get_latest(){
    ...
    $.post('/get_latest_single_intrusion',data = latest_data,function(result){
        if (result) {
...
window.operateEvents = {
    "click #img_btn":function(e,value,row,index){
        ...
        for(let i = 0; i < all_data.length; i++){
            img_list.push(all_data[i]['imr_url'])
        }
        $.pictureViewer({
